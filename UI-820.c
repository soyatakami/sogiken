// gcc -o UI-820.exe UI-820.c -lws2_32 -lcomctl32   ./UI-820.exe
// クライアント（UI 側）
// UI-820-1との違いは日数入力欄に数字しか入れられないこと
...
/////////////////////////////////
//半角数字しか入力できない．下の奴とどっちか
LRESULT CALLBACK DaysEditSubclassProc(
    HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam,
    UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    switch (msg) {
    case WM_CHAR:
        // 許可するキー：数字／BackSpace／Delete
        if ((wParam >= '0' && wParam <= '9') ||
            wParam == VK_BACK || wParam == VK_DELETE)
        {
            // 追加の文字が 5 桁超えそうなら弾く
            int len = GetWindowTextLengthA(hwnd);
            if (len >= 5 && (wParam >= '0' && wParam <= '9')) {
                MessageBeep(MB_ICONEXCLAMATION);
                return 0;
            }
            break;  // DefSubclassProc に流す
        }
        // その他は無効化
        MessageBeep(MB_ICONEXCLAMATION);
        return 0;
    ...
    case WM_PASTE:
        // クリップボードからテキスト取得→数字だけ抽出してセット
        if (OpenClipboard(hwnd)) {
            HANDLE h = GetClipboardData(CF_TEXT);
            if (h) {
                char *clip = (char*)GlobalLock(h);
                if (clip) {
                    char buf[6] = {0};
                    int p = 0;
                    for (int i = 0; clip[i] && p < 5; ++i) {
                        if (clip[i] >= '0' && clip[i] <= '9') {
                            buf[p++] = clip[i];
                        }
                    }
                    GlobalUnlock(h);
...
////////////////////////////
//10000より大きくしようとすると強制的に10000になる 上のやつどとっちか
/*
LRESULT CALLBACK DaysEditSubclassProc(
    HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam,
    UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    case WM_CHAR:
        ...
        if (wParam >= '0' && wParam <= '9') {
            // 1. 現在のテキスト取得
            char oldText[16] = {0};
            GetWindowTextA(hwnd, oldText, sizeof(oldText));

            // 2. キャレット位置取得／挿入後のテキスト構築
            DWORD selStart, selEnd;
            SendMessageA(hwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);

            char newText[16] = {0};
            int oldLen = lstrlenA(oldText);

            // 選択中なら上書き、そうでなければ挿入
            int pos = (int)selStart;
            if (selStart != selEnd) {
                // 選択部分を置き換え
                memcpy(newText, oldText, pos);
                newText[pos] = (char)wParam;
                strcpy(newText + pos + 1, oldText + selEnd);
            }
            else if (pos < oldLen) {
                // 挿入
                memcpy(newText, oldText, pos);
                newText[pos] = (char)wParam;
                strcpy(newText + pos + 1, oldText + pos);
            }
            else {
                // 末尾追加
                strcpy(newText, oldText);
                newText[oldLen] = (char)wParam;
                newText[oldLen+1] = '\0';
            }
            ...
            if (val > 10000) {
                // 範囲超え→強制クランプ
                const char *clamped = "10000";
                SetWindowTextA(hwnd, clamped);
                // キャレットを末尾に移動
                SendMessageA(hwnd, EM_SETSEL, 5, 5);
                return 0;  // 以降処理不要
            }

            // 4. 文字数オーバーもしくは "00000" 以上の長さ制限チェック
            if (lstrlenA(newText) > 5) {
                MessageBeep(MB_ICONEXCLAMATION);
                return 0;
            }
...

{
    ...
    g_hEditDays = CreateWindowA(
        ...
        // ここで 5 文字まで入力可能にする
SendMessageA(g_hEditDays, EM_SETLIMITTEXT, (WPARAM)5, 0);
...
// 日数入力欄のサブクラス化
// ここで 5 文字まで入力可能にする
                                 SetWindowSubclass(g_hEditDays, DaysEditSubclassProc, 0, 0);

// gcc -o UI-821.exe UI-821.c -lws2_32 -lcomctl32   ./UI-821.exe
// クライアント（UI 側）
...
static HWND CreateWindowModule(HINSTANCE hInst);
...
StatusFlags UISendReceive(const CorrectedData *corr, ResultData *res) {
    ...
    if (send_all(g_sock, (char*)buf, PACKET_SIZE) != PACKET_SIZE)
        return ++g_LastDaysErr.commErrorCount >= 5
            ? SF_COMM_NOT_CONNECTED
            : SF_COMM_GENERAL;

    if (recv_all(g_sock, (char*)buf, PACKET_SIZE) != PACKET_SIZE)
        return ++g_LastDaysErr.commErrorCount >= 5
            ? SF_COMM_NOT_CONNECTED
            : SF_COMM_GENERAL;
    ...
    StatusFlags         sf)
{
    ...
    if (sf & SF_ERR_DATE_REPLACED) {
        // corr に入っている「当日」の年・月・日を文字列化
        char tmp[128];
        snprintf(tmp, sizeof(tmp),
                 "開始日が範囲外のため、当日%04u年%02u月%02u日を開始日として計算します",
                 corr->year, corr->month, corr->day);
        strncpy(finalErr, tmp, sizeof(finalErr) - 1);
    }

    // 2) 入力エラー部分（小数切り上げ／切り捨て／上限超過）
    ...
    // 3) 通信エラー部分
    if (sf & SF_COMM_GENERAL) {
        strncat(finalErr,
                "\r\n通信に失敗しました。再度クリックしてください。",
                sizeof(finalErr) - strlen(finalErr) - 1);
    }
    else if (sf & SF_COMM_NOT_CONNECTED) {
        strncat(finalErr,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_COMMAND:
        ...
        case ID_BUTTON_CALC:
        {
            ...
            // 2) 入力値補正 + 入力エラーのフラグ取得
            status = InvalidValueHandling(&in, &corr);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nShow)
{
    ...
    HWND hwnd = CreateWindowModule(hInst);

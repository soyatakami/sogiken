...
//*今回は InvalidValueHandlingモジュールの切り離し
//*切り離し順は Main CreateWindow CollectData InvalidValueHandling

//*********************************************************************/
//*コンパイル方法
//gcc Coyomi_Client_Test_GUI_v20.c CreateWindow.c CollectData.c -o Coyomi_Client_Test_v20.exe -lcomctl32 -lws2_32 -mwindows -municode -std=c11 -O2 -Wall -Wextra -Wpedantic -finput-charset=CP932 -fexec-charset=CP932 -fwide-exec-charset=UTF-16LE
//******************************************************************* */

//*今回は CollectDataモジュールの切り離し
//*切り離し順は Main CreateWindow CollectData
...
//*********************************************************************/
//* 今回は InvalidValueHandling モジュールの切り離し
//* 切り離し順は Main → CreateWindow → CollectData → InvalidValueHandling
//*********************************************************************/

// コンパイル例：
// gcc Coyomi_Client_Test_GUI_v20.c CreateWindow.c CollectData.c InvalidValueHandling.c \
//   -o Coyomi_Client_Test_v20.exe -lcomctl32 -lws2_32 -mwindows -municode -std=c11 -O2 \
//   -Wall -Wextra -Wpedantic -finput-charset=CP932 -fexec-charset=CP932 -fwide-exec-charset=UTF-16LE

// Coyomi_Client_Test_GUI_v21.c
// 分離構成: Main / CreateWindow.c / CollectData.c / InvalidValueHandling.c
...
// InvalidValueHandling からの通知を受け取って 0→1 更新
...
static void start_request(HWND hwnd){
    ...
    // 2) 検証・補正は InvalidValueHandling に任せる
    ...
    InvalidValueHandling_InvalidValueHandling(
        ...
        NULL, 0,             // （ここでは文字列生成は不要。必要ならバッファ渡す）
